...
{
    public class ExtractNumberClaim : IProducePolicyMarkup
    {
        private readonly string _id, _elementToExtract;
        private readonly ClaimType _input, _output;
        public ExtractNumberClaim(string id, ClaimType input, string elementToExtract, ClaimType output)
        {
        ...
        private void ValidateInputs(ClaimType input, ClaimType output, string elem)
        {
            ...
            if (output.DataType != DataType.@long)
            {
                throw new Exception($"Output claim {output.Id} must be of type long; claim is of type {output.DataType}");
            }
        }
    }
}