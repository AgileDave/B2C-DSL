...
{
    public class ConvertDateTimeToDate : IProducePolicyMarkup
    {
        private readonly string _id;
        private readonly ClaimType _input, _output;

        public ConvertDateTimeToDate(string id, ClaimType input, ClaimType output)
        {
        ...
        {
            return TransformationBuilder.Init(_id, "ConvertDateTimeToDateClaim")
        ...
        {
            if (input.DataType != DataType.dateTime)
            {
                throw new Exception($"Input claim {input.Id} must be of type DateTime");
            }
            if (output.DataType != DataType.date)
            {
                throw new Exception($"Output claim {output.Id} must be of type Date");
            }
        }
